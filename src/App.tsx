...
export default function App() {
  ...
  const [viewMode, setViewMode] = useState<"app" | "canvas">("app");
  ...
  const currentUser = {
    displayName: user?.displayName || "Dra. Daniela Cázares",
    email: user?.email || "dra.daniela@cazaresdental.com",
      ...
      {viewMode === "canvas" ? (
        <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center py-8 px-4">
          <div className="w-full max-w-4xl flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-white">Previsualización Canvas de Cotización</h2>
            <button
              onClick={() => setViewMode("app")}
              className="px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-slate-950 rounded-lg text-xs font-semibold transition-colors cursor-pointer"
            >
              ← Volver al Dashboard
            </button>
          </div>
